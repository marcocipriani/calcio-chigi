"use client"

// lucide 1.x ha rimosso le brand icon (Github): SVG inline.
function GithubIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
      <path d="M12 .5C5.37.5 0 5.87 0 12.5c0 5.3 3.44 9.8 8.21 11.39.6.11.82-.26.82-.58v-2.03c-3.34.73-4.04-1.61-4.04-1.61-.55-1.39-1.34-1.76-1.34-1.76-1.09-.75.08-.73.08-.73 1.2.09 1.84 1.24 1.84 1.24 1.07 1.83 2.81 1.3 3.5.99.11-.78.42-1.3.76-1.6-2.67-.3-5.47-1.33-5.47-5.93 0-1.31.47-2.38 1.24-3.22-.12-.3-.54-1.52.12-3.17 0 0 1.01-.32 3.3 1.23a11.5 11.5 0 0 1 6 0c2.29-1.55 3.3-1.23 3.3-1.23.66 1.65.24 2.87.12 3.17.77.84 1.24 1.91 1.24 3.22 0 4.61-2.81 5.62-5.49 5.92.43.37.81 1.1.81 2.22v3.29c0 .32.22.7.83.58C20.56 22.3 24 17.8 24 12.5 24 5.87 18.63.5 12 .5z" />
    </svg>
  )
}

interface AppCreditsProps {
  uid?: string;
}

export function AppCredits({ uid }: AppCreditsProps) {
  const appVersion = process.env.NEXT_PUBLIC_APP_VERSION || "2.2.1";

  return (
    <div className="mt-8 flex w-full flex-col items-center justify-center gap-3 border-t border-border/50 py-8">
        
        {uid && (
            <code className="rounded bg-muted px-2 py-1 font-mono text-[11px] text-muted-foreground select-all">
                UID: {uid}
            </code>
        )}

        <div className="flex flex-col items-center gap-1">
            <p className="text-[11px] text-muted-foreground font-medium">
                Calcio Circolo Chigi <span className="font-mono text-[10px] bg-muted px-1 rounded">v{appVersion}</span>
            </p>
            <p className="text-[11px] text-muted-foreground flex items-center gap-1">
                Dev: <span className="font-bold">Marco Cipriani</span>
            </p>
            <p className="text-[10px] text-muted-foreground">
                Ultimo aggiornamento: 30 luglio 2026
            </p>
        </div>
        
        <a 
            href="https://github.com/marcocipriani/calcio-chigi"
            target="_blank" 
            rel="noopener noreferrer"
            className="flex items-center gap-1.5 text-[11px] text-muted-foreground hover:text-primary transition-colors bg-muted/30 px-3 py-1 rounded-full border border-transparent hover:border-border"
        >
            <GithubIcon className="h-3 w-3" />
            GitHub Repo
        </a>
    </div>
  )
}
